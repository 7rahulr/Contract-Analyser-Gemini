...
def get_gemini_response(input_text):
    ...
    # The prompt is updated to include 'Value of Contract', 'Payment Terms', and 'Project Timelines'.
    prompt = f"""
    ...
    {{
      ...
      "Value of Contract": "The total monetary value of the contract.",
      "Payment Terms": "A summary of payment terms, including amount, frequency, and method.",
      "Project Timelines": "Key project milestones, start date, and end date.",
                ...
                if contract_text:
                    with st.spinner('Analyzing contract... Please wait.⏳'):
                        ...
                        if analysis_data:
                            ...
                            # Custom CSS for the colored boxes and icons
                            st.markdown("""
                            <style>
                            .custom-box {
                                border-radius: 5px;
                            ...
                            .box-teal { background-color: #e5f2f2; }
                            .box-cyan { background-color: #e5f7ff; }
                            </style>
                            ...
                            box_colors = {
                                ...
                                "Value of Contract": "box-yellow",
                                "Payment Terms": "box-teal",
                                "Project Timelines": "box-cyan"
                            }
                            ...
                            # --- Clause Presence Checkboxes with Icons ---
                            st.subheader("Clause Presence Check ✅")
                            clauses = analysis_data.get("Clauses Presence", {})
                            
                            # Helper function to display an icon based on value
                            def get_icon(value):
                            ...
                            with col1:
                                st.markdown(f"{get_icon(commercial_clauses.get('Payment Terms', 'No'))} Payment Terms")
                                st.markdown(f"{get_icon(commercial_clauses.get('IP', 'No'))} IP (Intellectual Property)")
                            with col2:
                                st.markdown(f"{get_icon(commercial_clauses.get('Delivery Time', 'No'))} Delivery Time")
                                st.markdown(f"{get_icon(commercial_clauses.get('Warranty', 'No'))} Warranty")
                            ...
                            with col3:
                                st.markdown(f"{get_icon(legal_clauses.get('Indemnification', 'No'))} Indemnification")
                                st.markdown(f"{get_icon(legal_clauses.get('Termination', 'No'))} Termination")
                            with col4:
                                st.markdown(f"{get_icon(legal_clauses.get('Confidentiality', 'No'))} Confidentiality")
                                st.markdown(f"{get_icon(legal_clauses.get('Limitation of Liability', 'No'))} Limitation of Liability")
